<template>
	...
	<form @submit="save">
		...
		<van-cell-group inset :border='false'>
			...
			<van-cell center title="是否需要退费" :border='needRefund'>
				<van-switch size='20px' :checked="needRefund" @change='needRefund=$event.detail' />
			</van-cell>
			<template v-if="needRefund">
				<van-field focus :value='item.refund' @change='item.refund=$event.detail' right-icon='bill-o'
					label='退费金额' type='digit' input-align="right" placeholder='输入退费金额' />
				...
				<van-cell title="退费剩余天数" :border='false' v-if="!isRefund">
					<template v-if='eDays<0'>已超期<text style="margin-inline: 6px;color: crimson;">{{eDays * -1}}</text>天
					</template>
		...
		<div class="login-button">
			<van-button form-type='submit' :disabled="(!item.refund||item.refund<=0)&&needRefund" :loading='loading'
				block type="info" loading-type="circular">保 存</van-button>
		</div>
	</form>
</template>
...
<script lang="ts">
	...
	export default {
		...
		onLoad({
			...
			this.item.text = `<div style='font-size:16px'>${this.item.litigant?.replace(';', split).replace('；', split)}</div>`
